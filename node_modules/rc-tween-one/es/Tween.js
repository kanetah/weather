...
p.getComputedStyle = function () {
  ...
  if (this.isSvg && style.transform === 'none') {
    ...
    this.svgComputedStyle = _extends({}, style);
    return this.svgComputedStyle;
  }
  return style;
};
p.getAnimStartData = function (item) {
  ...
  Object.keys(item).forEach(function (_key) {
    if (_key in _plugin || _this4.attr === 'attr' && (_key === 'd' || _key === 'points')) {
    ...
    if (_this4.attr === 'attr') {
      ...
      if (_key.match(/color/i) || _key === 'stroke' || _key === 'fill') {
        ...
        start[_key] = unit !== item[_key].unit ? startConvertToEndUnit(_this4.target, _key, parseFloat(data), unit, item[_key].unit) : parseFloat(data);
      }
      return;
    }
...
p.setAnimData = function (data) {
  ...
  Object.keys(data).forEach(function (key) {
    if (key in _plugin || _this5.attr === 'attr' && (key === 'd' || key === 'points')) {
      return;
    }
...
p.setRatio = function (ratio, endData, i) {
  ...
  Object.keys(endData.vars).forEach(function (_key) {
    if (_key in _plugin || _this6.attr === 'attr' && (_key === 'd' || _key === 'points')) {
    ...
    if (_this6.attr === 'attr') {
      ...
      if (!endVars.type) {
        ...
        _this6.target.setAttribute(_key, getColor(data));
      }
    }
  });
...
p.render = function () {
  ...
  this.defaultData.forEach(function (item, i) {
    ...
    //  精度损失，只取小数点后10位。
    var progressTime = toFixed(_this7.progressTime - initTime);
    ...
    if (progressTime + fromDelay >= 0) {
      if (!_this7.start[i]) {
        ...
        if (progressTime < _this7.perFrame) {
          ...
          _this7.setRatio(toFixed(ratio), item, i);
